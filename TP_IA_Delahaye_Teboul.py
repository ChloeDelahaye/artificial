...
def searchAlgorithm(start, n):
    ...
    # MAJ heuristique et couts
            ...
            for x in range(len(tempList)):
                child = tempList[:]
                # on verifie si le noeud a deja ete explore
                for item in enumerate(explored):
                    if item == child:
                        find =True
                        heappush(pq, child)
                #on verifie si le noeud est deja dans la priority queue
                for item in enumerate(pq):
                    if item == child:
                        find=True
                        heappush(pq, child)
...
result4 = searchAlgorithm(t0, n)

# Compteur du nombre de déplacements
noofMoves = 0

# Calcul/Affichage du temps d'exécution
print("recherche avec UCS terminee en %s secondes" % (time.time() - start_time))
